...
- `Buổi trước` counts ranking matches in the latest calendar play day that has
  matches, using Vietnam date boundaries.
...
- The main dashboard may show the latest completed-season champion as a
  centered title-line honor under `Pickleball Ranking`. It links to
  `/analysis?zone=hall`, does not show a portrait on the main dashboard, and
  only appears when at least one completed-season champion exists.
