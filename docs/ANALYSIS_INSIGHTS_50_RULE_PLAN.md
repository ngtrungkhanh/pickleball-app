...
| 42 | `mental_block` | Opponent edge `total >= 4`, `impact <= -15`, and `rate <= 45` | rare / 1 | `${edge.playerName} mỗi khi đối đầu ${edge.otherName} thường có dấu hiệu bị "cóng" tâm lý, khiến kết quả thi đấu thực tế thấp hơn kỳ vọng từ ELO tới ${absRound(edge.impact)} điểm.` | | User text phrases impact as ELO expectation gap. |
| 43 | `sweet_matchup` | Opponent edge `total >= 4`, `impact >= 15`, and `rate >= 55` | rare / 1 | `${edge.playerName} mỗi khi chạm trán ${edge.otherName} thường thi đấu cực kỳ thăng hoa, giúp hiệu suất thực tế vượt xa kỳ vọng từ ELO tới ${edge.impact} điểm.` | | User text phrases impact as ELO expectation gap. |
