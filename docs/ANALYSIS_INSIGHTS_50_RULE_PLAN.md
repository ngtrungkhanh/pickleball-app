...
|---:|---|---|---|---|---|
| 1 | `hot_streak` | `streakType === W` and `streakCount >= 4` | occasional / 1 | `${metric.name} đang thắng liền ${metric.streakCount} trận, form này lên sân là đối thủ phải chuẩn bị thở oxy.` | Good. |
...
| 4 | `giant_killer` | `upsetWins > 0`; each upset win means expected win rate below 30% | rare / 1 | `${metric.name} có ${metric.upsetWins} lần thắng cửa dưới khi tỷ lệ thắng dự tính chỉ dưới 30%, đúng kiểu chuyên gạt giò.` | One upset is enough. |
| 5 | `earthquake_victim` | `upsetLosses > 0`; each upset loss means expected win rate above 70% | rare / 1 | `${metric.name} có ${metric.upsetLosses} lần cửa trên trên 70% mà vẫn rơi kèo, sân phủi đúng là khó đoán.` | One upset loss is enough. |
| 6 | `perfect_form5` | `total >= 5` and `formScore === 100` | occasional / 1 | `${metric.name} đang thắng 5/5 trận gần nhất, bảng form xanh kín nhìn khá cháy.` | Means latest 5 matches are all wins. |
...
| 8 | `gatekeeper` | `total >= 20` and `abs(rating - 1000) <= 20` | frequent / 0.55 | `${metric.name} đã đánh ${metric.total} trận mà ELO vẫn quanh ${metric.rating}, đúng kiểu người giữ cổng 1000.` | Good. |
...
| 15 | `perfect_duo` | Partner edge `total >= 4` and `rate >= 75` | occasional / 1 | `${edge.playerName} và ${edge.otherName} đang thắng ${edge.wins}/${edge.total} trận chung, đạt ${edgeRate(edge)}%, đúng chất cặp bài trùng.` | Directed edge from player perspective. |
| 16 | `bad_duo` | Partner edge `total >= 4` and `rate <= 25` | occasional / 1 | `${edge.playerName} đi với ${edge.otherName} mới thắng ${edge.wins}/${edge.total} trận, tỷ lệ ${edgeRate(edge)}%, dữ liệu đang báo hơi dẫm chân nhau.` | Directed edge from player perspective. |
| 17 | `partner_boost` | Partner edge `total >= 4`, `impact >= 15`, and `rate >= 50` | rare / 1 | `${edge.playerName} cặp với ${edge.otherName} thắng ${edge.wins}/${edge.total} trận và đánh cao hơn kỳ vọng từ ELO ${edge.impact} điểm.` | User text phrases impact as ELO expectation gap. |
| 18 | `partner_drag` | Partner edge `total >= 4`, `impact <= -15`, and `rate <= 40` | rare / 1 | `${edge.playerName} đứng cùng ${edge.otherName} chỉ thắng ${edge.wins}/${edge.total} trận, lại thấp hơn kỳ vọng từ ELO ${absRound(edge.impact)} điểm.` | User text phrases impact as ELO expectation gap. |
| 19 | `cover_master` | Player `total >= 8`, `attackScore <= 85`, `synergyScore >= 60`, and at least 2 partner edges with `total >= 4` | rare / 1 | `${metric.name} ghi điểm không quá ồn ào nhưng đi với nhiều đồng đội vẫn giúp cặp thắng ${round(metric.synergyScore)}% số trận.` | Review thresholds; attack cutoff may be too loose. |
| 20 | `carry_partner` | Partner edge `total >= 4`, edge win rate is at least 18 points above that player's overall win rate, and edge `rate >= 55` | rare / 1 | `${edge.playerName} đi với ${edge.otherName} thắng ${edge.wins}/${edge.total} trận, cao hơn hẳn mức thường thấy của ${edge.playerName}.` | Measures named player's lift, not reverse partner lift. |
...
| 22 | `stable_partner` | Partner edge `total >= 6`, `50 <= rate <= 65`, and `abs(impact) <= 5` | frequent / 0.55 | `${edge.playerName} và ${edge.otherName} đánh chung ${edge.total} trận, thắng ${edge.wins} trận, không bùng nổ nhưng khá tròn vai.` | Good. |
| 23 | `glued_pair` | Most frequent directional partner edge with `total >= 8` | frequent / 0.45 | `${glued.playerName} và ${glued.otherName} đã đánh chung ${glued.total} trận, tần suất dính nhau nhiều nhất sân.` | Directional edge can choose one side of a pair. |
| 24 | `rare_pair_hot` | Partner edge `total` between 4 and 5, and `rate >= 80` | rare / 1 | `${edge.playerName} và ${edge.otherName} mới đánh ${edge.total} trận nhưng thắng ${edge.wins}/${edge.total}, mẫu còn mỏng mà nhìn khá thơm.` | Because partner candidates are prefiltered at `total >= 4`. |
| 25 | `disaster_duo` | Partner edge `total >= 4`, `rate <= 35`, and `avgDiff <= -3` | rare / 1 | `${edge.playerName} và ${edge.otherName} thua ${edge.losses}/${edge.total} trận chung, trung bình mỗi trận âm ${oneDecimal(Math.abs(edge.avgDiff))} điểm, cần đổi bài gấp.` | Good. |
| 26 | `partner_long_games` | Partner edge `total >= 4` and `deuceGames >= 3` | occasional / 1 | `${edge.playerName} và ${edge.otherName} đánh chung mà đã có ${edge.deuceGames} trận kéo qua 11 điểm, cặp này thích cò cưa thật.` | User-facing text says `kéo qua 11 điểm`. |
...
| 36 | `low_score_magnet` | `lowScoreLosses >= 3`; low-score loss means losing team scored `<= 4` | occasional / 1 | `${metric.name} góp mặt trong ${metric.lowScoreLosses} trận team thua điểm rất thấp, đúng kiểu cột thu lôi hôm xấu trời.` | Plan wording should mention the `<= 4` threshold. |
...
| 41 | `boss_hunter` | `winsVsHigherElo >= 3` | rare / 1 | `${metric.name} có ${metric.winsVsHigherElo} lần thắng đối thủ ELO cao hơn, thợ săn trùm hơi uy tín.` | Uses wins against higher-ELO teams, not expected win rate. |
| 42 | `mental_block` | Opponent edge `total >= 4`, `impact <= -15`, and `rate <= 45` | rare / 1 | `${edge.playerName} gặp ${edge.otherName} thì đánh thấp hơn kỳ vọng từ ELO ${absRound(edge.impact)} điểm, dấu hiệu khớp kèo khá rõ.` | User text phrases impact as ELO expectation gap. |
| 43 | `sweet_matchup` | Opponent edge `total >= 4`, `impact >= 15`, and `rate >= 55` | rare / 1 | `${edge.playerName} gặp ${edge.otherName} đang thắng ${edge.wins}/${edge.total} trận và cao hơn kỳ vọng từ ELO ${edge.impact} điểm, kèo này khá thơm.` | User text phrases impact as ELO expectation gap. |
| 44 | `bully_lower_elo` | `totalVsLowerElo >= 8` and win rate vs lower-ELO teams `>= 70` | occasional / 1 | `${metric.name} thắng ${metric.winsVsLowerElo} trận trước nhóm ELO thấp hơn, farm kèo mềm khá đều tay.` | Review tone: `farm kèo mềm` may be too harsh if schedule is unavoidable. |
...
| 50 | `alternating_form` | `alternations >= 5` in latest result pattern | occasional / 1 | `Form gần đây của ${metric.name} nhảy ${pattern(metric.recentResults)} liên tục, đúng kiểu máy test vợt.` | Good. |
