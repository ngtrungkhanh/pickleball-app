...
| 4 | `giant_killer` | `upsetWins > 0`; each upset win means expected probability below 30% | rare / 1 | `${metric.name} có ${metric.upsetWins} lần thắng cửa dưới khi tỷ lệ thắng dự tính trước trận chỉ dưới 30%, đúng kiểu kèo khó vẫn lật được.` | **Copy variants**: `${metric.name} có ${metric.upsetWins} pha lật kèo ngoạn mục dù tỷ lệ thắng trước trận chỉ dưới 30%.`<br>`Chuyên gia lật kèo: cửa thắng trước trận chưa tới 30%, nhưng ${metric.name} vẫn có ${metric.upsetWins} lần gạt giò cửa trên.`<br>`Kèo khó không thành vấn đề: ${metric.name} đã ${metric.upsetWins} lần thắng ngược dù bị đánh giá cửa dưới rõ rệt.`<br>`${metric.name} có tới ${metric.upsetWins} pha gạt giò kinh điển khi bị xếp mâm dưới. Cực kỳ lỳ lợm.` | Copy-only proposed replacement; no trigger/frequency change. Avoid ELO/prediction-system wording here. |
| 5 | `earthquake_victim` | `upsetLosses > 0`; each upset loss means expected probability above 70% | rare / 1 | `${metric.name} có ${metric.upsetLosses} lần tỷ lệ thắng dự tính trước trận lên tới trên 70% mà vẫn rơi kèo.` | | One upset loss is enough. |
| 6 | `perfect_form5` | `total >= 5` and `formScore === 100` | occasional / 1 | `${metric.name} đang thắng 5 trận liên tiếp, bảng phong độ xanh kín nhìn khá cháy.` | | Means latest 5 matches are all wins. |
| 7 | `zero_form5` | `total >= 5` and `formScore === 0` | occasional / 1 | `${metric.name} đang thua 5 trận liên tiếp, bảng phong độ đỏ kín nhìn hơi chán.` | | Means latest 5 matches are all losses. |
...
| 12 | `revenge_win` | Best global opponent row with `meetings >= 4`, prior consecutive losses `>= 3`, and at least 1 recent win in latest 4 meetings | rare / 1 | `${player.name} cuối cùng cũng giải được ${opponent.name} sau ${revenge.priorLosses} lần thua trước đó, kèo phục hận đã lên sóng.` | **Chính**: `${player.name} cuối cùng cũng giải được ${opponent.name} sau ${revenge.priorLosses} lần thua trước đó, kèo phục hận đã lên sóng.` | Only strongest current candidate is emitted. |
...
| 14 | `rank_climber` | ELO rank improved by `>= 2` places versus approximated old ELO rank, and latest 5 has at least 3 wins | rare / 1 | `${metric.name} đang leo ${places} bậc trên bảng ELO gần đây, với ${recentWins} chiến thắng trong 5 trận mới nhất, đà thăng tiến đang cực kỳ ấn tượng.` | | Not true historical leaderboard rank yet. |
...
| 32 | `bagel_loss` | `bagelLosses > 0`; bagel loss means losing team scored `<= 2` | occasional / 1 | `${metric.name} có ${metric.bagelLosses} trận thua mà team chỉ ghi tối đa 2 điểm.` | | One occurrence is enough. |
| 33 | `clutch_master` | `closeWins >= 3` | occasional / 1 | `${metric.name} thắng sát nút ${metric.closeWins} trận, càng cuối kèo càng lì.` | | Close win means winning by 2 points (smallest Pickleball margin). |
...
| 45 | `victim_strong_elo` | `totalVsHigherElo >= 6` and loss rate vs higher-ELO teams `>= 65` | occasional / 1 | `${metric.name} gặp nhóm ELO cao hơn đang thua ${metric.lossesVsHigherElo}/${metric.totalVsHigherElo} trận, đụng độ "chiếu trên" quả thực không hề dễ nuốt.` | | Good. |
| 46 | `revenge_target` | Same source row as `revenge_win`: prior consecutive losses `>= 3`, then at least 1 win in latest 4 meetings | rare / 1 | `Gần đây ${player.name} gặp ${opponent.name} thắng ${revenge.recentWins}/${revenge.recentTotal} trận sau giai đoạn bị đì, có mùi lật kèo.` | **Chính**: `Gần đây ${player.name} gặp ${opponent.name} thắng ${revenge.recentWins}/${revenge.recentTotal} trận sau giai đoạn bị đì, có mùi lật kèo.` | Only strongest current candidate is emitted. |
| 47 | `iron_lung` | Top activity player by total matches, then daily max; requires `total >= 20` | frequent / 0.45 | `${metric.name} đang tạm dẫn đầu toàn sân về số lượng cày ải với ${metric.total} trận, phong độ cực kỳ bền bỉ.` | | Keep tone playful/persistent rather than dominant, since the gap over #2 could be just 1 match. |
