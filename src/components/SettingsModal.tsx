...
export function SettingsModal({ open, onClose, canEdit, onUnlock, onLock, players, seasons, config }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      />
      <div 
        className="relative w-full sm:max-w-4xl h-[92vh] sm:h-[640px] sm:rounded-3xl rounded-t-[2.5rem] border border-white/[0.08] bg-slate-950 shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col animate-in slide-in-from-bottom-10 duration-300"
      >
        <div className="flex items-center justify-between px-6 py-3.5 sm:py-5 border-b border-white/[0.06] shrink-0">
          <div>
            <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">Cài đặt</h2>
            <p className="text-[9px] sm:text-[10px] font-bold text-white/25 uppercase tracking-widest mt-0.5">
              {canEdit ? 'Đang có quyền chỉnh sửa' : 'Nhập mật khẩu để mở khóa'}
            </p>
          </div>
          <button onClick={onClose} className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/50 transition-all active:scale-90">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col sm:grid sm:grid-cols-[220px_1fr]">
          ...
          <div className="shrink-0 p-2 sm:p-4 border-b border-white/[0.06] bg-white/[0.01] sm:bg-white/[0.02]">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-10 space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
                <div className="flex items-center gap-4 mb-5">
                  <div className={cn(
                    ...
                    canEdit ? "bg-primary/10 border-primary/20 text-primary" : "bg-white/5 border-white/10 text-white/20"
                  )}>
                    <ShieldCheck className="w-6 h-6" />
                  </div>
                  <div>
                    <div className="font-black text-white text-lg tracking-tight">{canEdit ? 'Quyền chỉnh sửa đang mở' : 'Mở quyền chỉnh sửa'}</div>
                    <p className="text-xs font-bold text-white/30 uppercase tracking-widest">Xác thực để thay đổi dữ liệu</p>
                  </div>
                </div>
                ...
                {!canEdit ? (
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input
                      ...
                      className="flex-1 rounded-2xl bg-slate-900/50 border border-white/[0.08] px-5 py-4 text-white outline-none focus:border-primary/50 focus:bg-slate-900 transition-all"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                  ...
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className="rounded-2xl bg-white/5 hover:bg-white/10 px-8 py-4 text-xs font-black text-white/60 uppercase tracking-widest transition-all active:scale-95"
                    >
                      Khóa lại
                    </button>
                  </div>
            ...
            {canEdit && activeTab === 'players' && (
              <div className="space-y-4">
                  <div className="space-y-2">
                    <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] px-1">Thêm thành viên</div>
                    <form action={(fd) => submit(addPlayerAction, fd, 'add-player', 'Đã thêm')} className="flex gap-2">
                      <input name="name" placeholder="Tên thành viên..." className="flex-1 rounded-xl bg-slate-900/50 border border-white/[0.08] px-4 py-2.5 text-sm text-white outline-none focus:border-primary/50 transition-all" />
                  ...
                  <div className="space-y-2">
                    <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] px-1">Danh sách thành viên</div>
                      ...
                      {[...players].sort((a, b) => (a.active === b.active ? 0 : a.active ? -1 : 1)).map(p => (
                        <div key={p.id} className={cn(
                          ...
                          p.active !== false 
                            ...
                            : "bg-black/20 border-white/[0.02] opacity-40"
                        )}>
                          ...
                          <label className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-white/5 rounded-lg transition-colors shrink-0">
                            <input 
                              type="checkbox" 
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <div className="text-[10px] font-black text-white/25 uppercase tracking-[0.2em] px-1">Tạo Season mới</div>
                    <form action={(fd) => submit(createSeasonAction, fd, 'create-season', 'Đã tạo')} className="flex gap-2">
                      <input name="name" placeholder="Tên Season..." className="flex-1 rounded-2xl bg-slate-900/50 border border-white/[0.08] px-4 py-3 text-sm text-white outline-none focus:border-primary/50 transition-all" />
                  ...
                  <div className="space-y-3">
                    <div className="text-[10px] font-black text-white/25 uppercase tracking-[0.2em] px-1">Kết thúc nhanh</div>
                    <button 
                      onClick={() => submit(endSeasonAction, new FormData(), 'end-season', 'Đã bắt đầu Season mới')}
                      disabled={isPending}
                      className="w-full rounded-2xl bg-white/5 hover:bg-white/10 border border-white/[0.08] px-5 py-3 text-[10px] font-black text-white/60 hover:text-white uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2"
                    >
                ...
                <div className="space-y-3">
                  <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] px-1">Danh sách Season</div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {!s.active && (
                            <form action={(fd) => submit(setActiveSeasonAction, fd, `season-${s.name}`, 'Đã kích hoạt')}>
                              ...
                              <button disabled={isPending} className="rounded-lg bg-white/5 hover:bg-white/10 px-3 py-1.5 text-[9px] font-black text-white/50 hover:text-white uppercase tracking-widest transition-all">
                                Kích hoạt
                              </button>
                            </form>
                          )}
            ...
            {canEdit && activeTab === 'money' && (
              <div className="space-y-4">
                <div className="text-[10px] font-black text-white/25 uppercase tracking-[0.2em] px-1">Cấu hình tài chính</div>
                <form action={(fd) => submit(updateFineAction, fd, 'fine', 'Đã lưu')} className="space-y-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
                  <div className="space-y-2">
                    <label className="text-xs font-black text-white/40 uppercase tracking-widest">Mức phạt mỗi lượt thua</label>
                    <div className="relative">
                      <input name="lose_money" type="number" defaultValue={config.lose_money || '5000'} className="w-full rounded-2xl bg-slate-900/50 border border-white/[0.08] px-5 py-4 text-white font-black text-xl outline-none focus:border-primary/50 transition-all pl-12" />
                      <div className="absolute left-5 top-1/2 -translate-y-1/2 text-white/20 font-black text-lg">₫</div>
                    </div>
                  </div>
        ...
        {/* Delete Player Modal */}
        {deleteTarget && (
          <div className="absolute inset-0 z-[600] flex items-center justify-center bg-black/80 backdrop-blur-md p-6">
            <div className="w-full max-w-sm rounded-[2.5rem] border border-red-500/20 bg-slate-950 p-8 shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="flex flex-col items-center text-center gap-6">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm font-bold text-white/30 leading-relaxed">
                ...
                <div className="flex gap-3 w-full">
                  <button onClick={() => setDeleteTarget(null)} className="flex-1 rounded-2xl bg-white/5 hover:bg-white/10 px-4 py-4 text-xs font-black text-white/40 uppercase tracking-widest transition-all">Hủy</button>
                  <button
                    onClick={() => {
                      const fd = new FormData();
          ...
          <div className="absolute inset-0 z-[600] flex items-center justify-center bg-black/80 backdrop-blur-md p-6">
            <div className="w-full max-w-sm rounded-[2.5rem] border border-red-500/20 bg-slate-950 p-8 shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="flex flex-col items-center text-center gap-6">
                ...
                <div className="flex gap-3 w-full">
                  <button onClick={() => setDeleteSeasonTarget(null)} className="flex-1 rounded-2xl bg-white/5 hover:bg-white/10 px-4 py-4 text-xs font-black text-white/40 uppercase tracking-widest transition-all">Hủy</button>
                  <button
                    onClick={() => {
                      const fd = new FormData();
